    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <label for="profile_picture" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 cursor-pointer">
                    {{ __('Choose File') }}
                </label>
                <span id="file_name" class="ml-2 text-sm text-gray-500">{{ auth()->user()->profile_picture ? basename(auth()->user()->profile_picture) : __('No file chosen') }}</span>
                <x-input-error class="mt-2" :messages="$errors->get('profile_picture')" />
                <p class="mt-1 text-sm text-gray-600">
                    {{ __('Upload a new profile picture (max 2MB).') }}
                </p>
            </div>
        </div>
        ...
        <script>
            function updateFileName(input) {
                ...
                if (input.files && input.files.length > 0) {
                    ...
                    fileNameSpan.textContent = '{{ __('No file chosen') }}';
                }
            }
        </script>
        ...
        <div>
            ...
            <x-text-input id="phone" name="phone" type="text" class="mt-1 block w-full" :value="old('phone', $user->phone)" autocomplete="tel" /> {{-- type="tel" for semantic HTML --}}
